...
def kkI(healey85data,What_is_limiting):   #this function calculate for the same irradiance
    ...
    Cnbiosynth=1.2*4.34728279914354E-10        #(molN cell-1 s) Constant for varible part of biosynthesis protein nitrogen (193-37)
    Nconst_protein=1.15*4.45336898828389E-15     #(molN cell-1) Constant protein pool in nitrogen (193-25)
    Nstore_max=2.91679384515998E-15         #(molN cell-1) Constant protein pool in nitrogen (193-25)
    Cnrna_variable=8.5*6212.59249917364        #(s) Constant for Variable part of RNA (193-26)
    ...
    Pconst_other=5.44534485638617E-22               #(molP cell-1) Constant part of phosphorus (193-26) * This includes ATP ADP, Phospholipid and DNA RNA
    Qp_max=0.4*1.5*25.26/(3.097e16)                                                                              #(molP cell-1) total phosphorus content in the cell (193-26)
...
############################ Elemental Stoichiometry #######################################
    ...
    for i in arange(U):
         j = where(growth_micro==growth_micro.min())[0][0]
         growth_plot[i] = growth_micro[j]
         ###################################
         RNA_plot[i] = RNA_micro[j]
         DNA_plot[i]=DNA_micro[j]
         Pro_plot[i]=Pro_micro[j]
         Pig_plot[i]=Pig_micro[j]
         Carb_plot[i]=Carb_micro[j]
         Lip_plot[i]=Lip_micro[j]
         ##################################
         RNA_Nplot[i] = RNA_Nm[j]
         DNA_Nplot[i]=DNA_Nm[j]
         Pro_Nplot[i]=Pro_Nm[j]
         Pig_Nplot[i]=Pig_Nm[j]
         ##########################
         RNA_Pplot[i] = RNA_Pm[j]
         DNA_Pplot[i] = DNA_Pm[j]
         Lip_Pplot[i] = Lip_Pm[j]
         Res_Pplot[i] = Res_Pm[j]
         ###########################
         NC[i]=CN_micro[j]
         PC[i]=CP_micro[j]
         ###########################
...
#     NP=NP[5:]
    ...
    carbs_m=array([carbs[4],carbs[11],carbs[24],carbs[119],carbs[359]])
    lipids_m=array([lipids[4],lipids[11],lipids[24],lipids[119],lipids[359]])
    carb_lip_m=array([carb_lip[4],carb_lip[11],carb_lip[24],carb_lip[119],carb_lip[359]])
    pigment_m=array([pigment[4],pigment[11],pigment[24],pigment[119],pigment[359]])
    protein_m=array([protein[4],protein[11],protein[24],protein[119],protein[359]])
    RNA_m=array([RNA[4],RNA[11],RNA[24],RNA[119],RNA[359]])
    DNA_m=array([DNA[4],DNA[11],DNA[24],DNA[119],DNA[359]])
    ...
    carbs_lip=data_carbs+data_lip
    #print(carbs_lip)
    
    carbs_d=array([data_carbs[2],data_carbs[6],data_carbs[9],data_carbs[11],data_carbs[14]])
    lipids_d=array([data_lip[2],data_lip[6],data_lip[9],data_lip[11],data_lip[14]])
    carbs_lip_d=array([carbs_lip[2],carbs_lip[6],carbs_lip[9],carbs_lip[11],carbs_lip[14]])
    pigment_d=array([data_pig[2],data_pig[6],data_pig[9],data_pig[11],data_pig[14]])
    protein_d=array([data_pro[2],data_pro[6],data_pro[9],data_pro[11],data_pro[14]])
    RNA_d=array([data_RNA[2],data_RNA[6],data_RNA[9],data_RNA[11],data_RNA[14]])
    DNA_d=array([data_DNA[2],data_DNA[6],data_DNA[9],data_DNA[11],data_DNA[14]])

    
    growth_rate = ['0.005', '0.012', '0.025', '0.12', '0.36']
