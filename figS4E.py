...
def kkI(healey85data,What_is_limiting):   #this function calculate for the same irradiance
    ...
    m=1.3*3.79146798299876E-19         #(mol C s-1 cell-1) maintenance carbonhydrate consumption (idea from 172-7)
    ...
    Cnbiosynth=0.9*4.34728279914354E-10        #(molN cell-1 s) Constant for varible part of biosynthesis protein nitrogen (193-37)
    Nconst_protein=0.1*4.45336898828389E-15     #(molN cell-1) Constant protein pool in nitrogen (193-25)
    Nstore_max=2.91679384515998E-15         #(molN cell-1) Constant protein pool in nitrogen (193-25)
    Cnrna_variable=5*6212.59249917364        #(s) Constant for Variable part of RNA (193-26)
...
#OOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOOO
    ...
    RNA_Cmaure=RNA_maure*(10.7/15.5)
    DNA_Cmaure=DNA_maure*(11.1/15.9)
    Pro_Cmaure=Pro_maure*(4.49/5.49)
    Pig_Cmaure=Pig_maure*(55/59)
    Carb_Cmaure=Carb_maure
    Lip_Cmaure=Lip_maure-Pinlipid_maure
    ...
    Growth_r=size(gr_rosco)  
    mari_growth_plot = zeros(Growt)
    maure_growth_plot = zeros(Growth)
    rosco_growth_plot = zeros(Growth_r)
    mari_NC= zeros(Growt)
    maure_NC= zeros(Growth)
    rosco_NC= zeros(Growth_r)
    mari_PC= zeros(Growt)
    maure_PC= zeros(Growth)
    rosco_PC= zeros(Growth_r)
    mari_NP= zeros(Growt)
    maure_NP= zeros(Growth)
    rosco_NP= zeros(Growth_r)
    ...
    #phosphorus
    maure_RNA_Pplot = zeros(Growth)
    maure_DNA_Pplot = zeros(Growth)
    maure_Lip_Pplot = zeros(Growth)
    maure_Res_Pplot = zeros(Growth)
    ...
    for i in arange(Growth):
        ...
        #########################
        maure_RNA_Pplot[i] = RNA_Pmaure[j]
        maure_DNA_Pplot[i] = DNA_Pmaure[j]
        maure_Lip_Pplot[i] = Lip_Pmaure[j]
        maure_Res_Pplot[i] = Res_Pmaure[j]
        gr_maure[j] = 1e30 
    ...
    N_pigment_m=array([ N_pigment[133], N_pigment[158], N_pigment[250], N_pigment[366], N_pigment[620]])
    N_protein_m=array([N_protein[133],N_protein[158],N_protein[250],N_protein[366],N_protein[620]])
    N_RNA_m=array([N_RNA[133],N_RNA[158],N_RNA[250],N_RNA[366],N_RNA[620]])
    N_DNA_m=array([N_DNA[133],N_DNA[158],N_DNA[250],N_DNA[366],N_DNA[620]])
     
    #observations
    N_data_pig=maure_Pig_Nplot*maure_NC
    N_data_pro=maure_Pro_Nplot*maure_NC
    N_data_RNA=maure_RNA_Nplot*maure_NC
    N_data_DNA=maure_DNA_Nplot*maure_NC
    #print(N_data_pro)

    N_pigment_d=array([N_data_pig[3],N_data_pig[5],N_data_pig[7],N_data_pig[9],N_data_pig[14]])
    N_protein_d=array([N_data_pro[3],N_data_pro[5],N_data_pro[7],N_data_pro[9],N_data_pro[14]])
    N_RNA_d=array([N_data_RNA[3],N_data_RNA[5],N_data_RNA[7],N_data_RNA[9],N_data_RNA[14]])
    N_DNA_d=array([N_data_DNA[3],N_data_DNA[5],N_data_DNA[7],N_data_DNA[9],N_data_DNA[14]])
