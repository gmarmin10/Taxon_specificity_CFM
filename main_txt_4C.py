...
def kkI(healey85data,What_is_limiting):   #this function calculate for the same irradiance
    ...
    m=1.1*3.79146798299876E-19         #(mol C s-1 cell-1) maintenance carbonhydrate consumption (idea from 172-7)
    Pmax=1.5*0.00320513285659728
    ...
    Nconst_protein=0.3*4.45336898828389E-15     #(molN cell-1) Constant protein pool in nitrogen (193-25)
    Nstore_max=2.91679384515998E-15         #(molN cell-1) Constant protein pool in nitrogen (193-25)
    Cnrna_variable=1.7*6212.59249917364        #(s) Constant for Variable part of RNA (193-26)
...
############################ Elemental Stoichiometry #######################################
    ...
    for i in arange(U):
        ...
        growth_plot[i] = growth_ps[j]
        #######################
        RNA_plot[i] = RNA_ps[j]
        DNA_plot[i]=DNA_ps[j]
        Pro_plot[i]=Pro_ps[j]
        Pig_plot[i]=Pig_ps[j]
        Carb_plot[i]=Carb_ps[j]
        Lip_plot[i]=Lip_ps[j]
        #########################
        RNA_Nplot[i] = RNA_Nps[j]
        DNA_Nplot[i]=DNA_Nps[j]
        Pro_Nplot[i]=Pro_Nps[j]
        Pig_Nplot[i]=Pig_Nps[j]
        #########################
        RNA_Pplot[i] = RNA_Pps[j]
        DNA_Pplot[i] = DNA_Pps[j]
        Lip_Pplot[i] = Lip_Pps[j]
        Res_Pplot[i] = Res_Pps[j]
        ########################
        NC[i]=CN_ps[j]
        PC[i]=CP_ps[j]
        ###########################
    ...
    growth_Cplot=delete(growth_plot,[9])
    Lip_plot=delete(Lip_plot,[9])
    RNA_plot = delete(RNA_plot, [9])
    DNA_plot = delete(DNA_plot, [9])
    Pro_plot = delete(Pro_plot, [9])
    Pig_plot = delete(Pig_plot, [9])
    Carb_plot = delete(Carb_plot, [9])
...
#     NP=NP[5:]
    ...
    carbs_m=array([carbs[0],carbs[249],carbs[560],carbs[699],carbs[929]])
    lipids_m=array([lipids[0],lipids[249],lipids[560],lipids[699],lipids[929]])
    pigment_m=array([pigment[0],pigment[249],pigment[560],pigment[699],pigment[929]])
    protein_m=array([protein[0],protein[249],protein[560],protein[699],protein[929]])
    RNA_m=array([RNA[0],RNA[249],RNA[560],RNA[699],RNA[929]])
    DNA_m=array([DNA[0],DNA[249],DNA[560],DNA[699],DNA[929]])
    carb_lip_m=array([carb_lip[0],carb_lip[249],carb_lip[560],carb_lip[699],carb_lip[929]])
    ...
    carbs_d=array([data_carbs[5],data_carbs[6],data_carbs[9],data_carbs[12],data_carbs[13]])
    lipids_d=array([data_lip[5],data_lip[6],data_lip[9],data_lip[12],data_lip[13]])
    carbs_lip_d=array([data_lip_carb[5],data_lip_carb[6],data_lip_carb[9],data_lip_carb[12],data_lip_carb[13]])
    pigment_d=array([data_pig[5],data_pig[6],data_pig[9],data_pig[12],data_pig[13]])
    protein_d=array([data_pro[5],data_pro[6],data_pro[9],data_pro[12],data_pro[13]])
    RNA_d=array([data_RNA[5],data_RNA[6],data_RNA[9],data_RNA[12],data_RNA[13]])
    DNA_d=array([data_DNA[5],data_DNA[6],data_DNA[9],data_DNA[12],data_DNA[13]])

    
    growth_rate = ['0.001', '0.25', '0.56', '0.70', '0.93']
